...
export class GameEngine {
  ...
  constructor() {
    ...
    this.gameStatus = GAME_STATUS.GAME_STARTED;
  ...
  protected _getAvailableMoves(color: Color, tileKey: string): string[] {
    ...
    if (playersPiece.type !== PIECE_TYPE.KING) {
  ...
  protected checkAndSetGameStatus() {
    ...
    if (this.isKingNowInCheck(enemyColor)) {
      if (!this.enemyHasAvailableMoves(enemyColor)) {
        return this.setGameStatus(GAME_STATUS.CHECKMATE);
      }
      return this.setGameStatus(GAME_STATUS.CHECK);
    }
    this.setGameStatus(GAME_STATUS.ONGOING);
  }
